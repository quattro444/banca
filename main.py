...
@app.get("/debug/gist", response_class=HTMLResponse)
def debug_gist():
    if not _gist_enabled():
        return HTMLResponse("Gist DISABILITATO (manca GIST_ID/GIST_TOKEN)", status_code=500)
    try:
        meta = _http_request(
            f"https://api.github.com/gists/{GIST_ID}",
            headers={"Authorization": f"Bearer {GIST_TOKEN}", "Accept": "application/vnd.github+json", "User-Agent": "bonsaura-app"}
...
def buy(request: Request, item_code: str = Form(...)):
    ...
    def debug_gist():
        ...
        try:
            ...
            f = files.get(GIST_FILENAME)
            if not f:
                return HTMLResponse(f"Gist OK ma file {GIST_FILENAME} assente", status_code=200)
            size = -1
            if f.get("truncated") and f.get("raw_url"):
                size = len(_http_request(f["raw_url"]))
            else:
                size = len((f.get("content") or "").encode("utf-8"))
            return HTMLResponse(f"Gist OK. {GIST_FILENAME} presente (size={size} bytes).", status_code=200)
        except Exception as e:
            return HTMLResponse(f"Errore Gist: {e}", status_code=500)
    return RedirectResponse(url="/shop", status_code=302)

...
import os
import base64, json, threading, tempfile
from urllib import request as urlrequest

# ---------- CONFIG ----------
# DB locale; se redeploy su Render resetta, per questo usiamo snapshot su Gist (vedi sotto)
...
ADMIN_KEY = "bunald"   # CAMBIA questa stringa prima del deploy
    ...
    # ---------- DB INIT ----------
    def init_db():
        # Assicura che la directory del DB esista (utile quando DB_FILE = /data/cards.db)
    ...
    # Tabella acquisti/abbonamenti shop
    c.execute("""
    ...
    def leaderboard(request: Request):
        # Accesso consentito solo subito dopo il tap NFC
        sid = request.cookies.get(SESSION_COOKIE_NAME)
        if not sid:
            return render_page(f"<h3>Sessione mancante</h3><p>Avvicina la carta NFC per accedere (entro {SCAN_WINDOW}s).</p>", "Richiesto NFC")
        session = get_session_info(sid)
        ...
        # Carica classifica per saldo (discendente)
        conn = sqlite3.connect(DB_FILE); c = conn.cursor()
    ...
    def apply_recurring_charges(token: str):
        ...
        for pid, item_name, weekly, next_ts in rows:
            ...
            if charges > 0:
                ...
                # scala saldo e aggiorna next_charge_at
                ...
                reason = f"Addebito {item_name} (-{weekly:.0f}/settimana) x{charges}"
                # log transazione verso "Negozio"
                c.execute(
                    "INSERT INTO transactions (ts, from_token, from_name, to_token, to_name, amount, reason) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (now, token, from_name, None, "Negozio", amount, reason)
                )

        conn.commit()
        conn.close()

    # SHOP: pagina del negozio (accesso solo con saldo sufficiente)
    @app.get("/shop", response_class=HTMLResponse)
    def shop(request: Request):
        # Accesso entro SCAN_WINDOW
        sid = request.cookies.get(SESSION_COOKIE_NAME)
        if not sid:
            return render_page(f"<h3>Sessione mancante</h3><p>Avvicina la carta NFC per accedere (entro {SCAN_WINDOW}s).</p>", "Richiesto NFC")
        session = get_session_info(sid)
    ...
    def buy(request: Request, item_code: str = Form(...)):
        ...
        # Log transazione di acquisto
        c.execute(
            "INSERT INTO transactions (ts, from_token, from_name, to_token, to_name, amount, reason) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (now, token, site["name"], None, "Negozio", -35.0, "Acquisto Moccolone (+35 subito, -3/settimana)")
        )
        conn.commit(); conn.close()
